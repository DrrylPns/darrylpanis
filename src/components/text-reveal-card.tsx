...
export const TextRevealCard = ({
  ...
  useEffect(() => {
    ...
    if (cardElement) {
      const { left, width: localWidth } = cardElement.getBoundingClientRect();
      setLeft(left);
  ...
  return (
    <div
      ...
      ref={cardRef}
      className={cn(
        "bg-[#f4c644] border border-white/[0.08] w-full rounded-lg p-8 relative overflow-hidden",
        className
      )}
    >
      {children}
      ...
      <div className="h-40 relative flex justify-center items-center overflow-hidden">
        <motion.div
          ...
          className="absolute bg-[#f4c644] z-20  will-change-transform"
        >
          <p
            style={{
        ...
        <div className="overflow-hidden [mask-image:linear-gradient(to_bottom,transparent,white,transparent)]">
          <p className="text-base sm:text-[3rem] py-10 font-bold bg-clip-text text-transparent bg-[#2a2a2f]">
            {text}
          </p>
          <MemoizedStars />
        </div>
